...
Extracted verbatim from the original monolithic data_cleaning.py (source lines 1-260). Logic is unchanged; this file is executed as one stage of the pipeline by run_pipeline.py, sharing a single namespace across all stages so variable state flows exactly as it did in the original script.
"""
...
# RAN_CLEAN_SHOW_PLOTS=1 (or pass --show-plots to the CLI) to restore interactive windows.
import matplotlib
if os.environ.get("RAN_CLEAN_SHOW_PLOTS", "").strip().lower() not in {"1", "true", "yes"}:
    matplotlib.use("Agg")
...
DATA_DIR = Path(os.environ.get("RAN_CLEAN_DATA_DIR", "Raw_Data"))
DRIVE_TEST_PATH = Path(os.environ["RAN_CLEAN_INPUT_FILE"]) if os.environ.get("RAN_CLEAN_INPUT_FILE") else DATA_DIR / "Network_Drive_Test.pkl"
OUTPUT_ROOT = Path(os.environ.get("RAN_CLEAN_OUTPUT_DIR", "Throughput_Anomaly_Detection_Outputs"))
