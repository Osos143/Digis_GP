...
Extracted verbatim from the original monolithic data_cleaning.py (source lines 261-844). Logic is unchanged; this file is executed as one stage of the pipeline by run_pipeline.py, sharing a single namespace across all stages so variable state flows exactly as it did in the original script.
"""
...
def save_plotly_html(fig, filename: str) -> None:
    ...
    try:
        ...
        print(f"Saved Plotly HTML: {path}")
    except Exception as e:
        print(f"Plotly save failed: {e}")
...
def plot_distribution_with_lines(
    ...
    try:
        ...
        max_y = counts.max() if len(counts) > 0 else 100
        if thresholds:
            for label, value in thresholds.items():
                if pd.notna(value):
                    fig.add_trace(go.Scatter(x=[float(value), float(value)], y=[0, max_y], mode='lines', line=dict(dash='dash'), name=f"{label}: {float(value):.2f}"))
        fig.update_layout(
...
def plot_box_by_category(
    ...
    try:
        fig = go.Figure()
        for cat_label, arr in zip(labels, data):
            fig.add_trace(go.Box(y=arr, name=cat_label))
        fig.update_layout(
            template='plotly_white',
            title=title,
...
def plot_scatter_sample(
    ...
    try:
        fig = go.Figure()
        fig.add_trace(go.Scattergl(x=tmp[x_col], y=tmp[y_col], mode='markers', marker=dict(size=6, opacity=0.35)))
        fig.update_layout(
            template='plotly_white',
            title=title,
            xaxis_title=xlabel,
...
def plot_route_anomaly_overlay(
    ...
    try:
        ...
        if not normal.empty:
            fig.add_trace(go.Scattergl(
                x=normal["longitude"], y=normal["latitude"],
                mode='markers', name="Not flagged",
                marker=dict(size=5, opacity=0.20, color='blue')
            ))
        if not anom.empty:
            score = pd.to_numeric(anom.get("anomaly_score_0_100", pd.Series(1, index=anom.index)), errors="coerce").fillna(1)
            fig.add_trace(go.Scattergl(
                x=anom["longitude"], y=anom["latitude"],
                mode='markers', name="Flagged anomaly",
                marker=dict(
                    size=10, opacity=0.85, 
                    color=score, colorscale='Viridis',
...
def plot_actual_vs_expected(df: pd.DataFrame, filename: Optional[str] = None, max_points: int = 25_000) -> None:
    ...
    try:
        ...
        if not normal.empty:
            fig.add_trace(go.Scattergl(
                x=normal["expected_tp_p75"], y=normal["actual_lte_dl_throughput"],
                mode='markers', name="Not flagged",
                marker=dict(size=5, opacity=0.25, color='blue')
            ))
        if not anom.empty:
            fig.add_trace(go.Scattergl(
                x=anom["expected_tp_p75"], y=anom["actual_lte_dl_throughput"],
                mode='markers', name="Flagged anomaly",
                marker=dict(size=7, opacity=0.55, color='orange')
            ))
        fig.add_trace(go.Scattergl(
            x=[0, max_axis], y=[0, max_axis],
            mode='lines', name="Actual = expected",
...
def plot_anomaly_context(
    ...
    try:
        ...
        if "download_analysis_window_flag" in ctx.columns:
            ...
            if inactive.any():
                y_marker = max(0, np.nanmin(ctx["actual_lte_dl_throughput"]) if ctx["actual_lte_dl_throughput"].notna().any() else 0)
                fig.add_trace(go.Scatter(x=ctx.loc[inactive, "context_sample_number"], y=np.repeat(y_marker, inactive.sum()), mode='markers', name="Outside parsed active download window", marker=dict(symbol='x', size=8)))
        ...
        if filename_prefix:
            save_plotly_html(fig, f"{filename_prefix}_rank_{rank + 1}_throughput_context.html")
    except Exception as e:
        print(f"Plotly generation failed: {e}")
    plt.show()
    ...
    if radio_cols:
        ...
        try:
            fig = go.Figure()
            for col in radio_cols:
                fig.add_trace(go.Scatter(x=ctx["context_sample_number"], y=ctx[col], mode='lines+markers', name=col, marker=dict(size=3)))
            
            y_min = ctx[radio_cols].min().min()
            y_max = ctx[radio_cols].max().max()
            fig.add_trace(go.Scatter(x=[center_x, center_x], y=[y_min if pd.notna(y_min) else 0, y_max if pd.notna(y_max) else 100], mode='lines', line=dict(dash='dash', color='red'), name="Selected anomaly"))
            ...
            if filename_prefix:
                save_plotly_html(fig, f"{filename_prefix}_rank_{rank + 1}_radio_context.html")
        except Exception as e:
            print(f"Plotly generation failed: {e}")
        plt.show()
